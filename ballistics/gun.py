...
class Gun:
    ...
    @property
    def total_charge_mass(self):
        return sum(self.charge_masses)

    @property
    def total_charge_volume(self):
    ...
    def _to_burnout(
        ...
        """
        Underlying method that implements integrating up to the burnout point, with
        provision for premature termination based on additional criterias passed by
        `Gun.to_travel` and `Gun.to_velocity`. Also see `Gun.to_burnout` for additional
        documentation.
        ...
        Notes
        -----
        This is only intended to be used when called by `Gun.to_velocity`,
        `Gun.to_burnout` or `Gun.to_travel`.
    ...
    def to_burnout(self, n_intg: int, acc: float) -> List[State]:
        ...
        Notes
        -----
        Implementation wise, calls `Gun._to_burnout` and passes the resulting states
        through `Gun.mark_max_pressure` before returning.
        """
        return self.mark_max_pressure(self._to_burnout(n_intg=n_intg, acc=acc), acc=acc)
    ...
    def to_travel(self, travel: float, n_intg: int, acc: float) -> List[State]:
        ...
        from burnout point to muzzle exit. Calls `Gun.to_burnout` for integration up
        ...
        n_intg: int
            ...
            in addition to being passed through to `Gun.to_burnout`.
        acc: float
        ...
        states = self._to_burnout(n_intg=n_intg, acc=acc, abort_travel=travel)
    ...
    def to_velocity(self, velocity: float, n_intg: int, acc: float) -> List[State]:
        ...
        states = self._to_burnout(n_intg=n_intg, acc=acc, abort_velocity=velocity)
